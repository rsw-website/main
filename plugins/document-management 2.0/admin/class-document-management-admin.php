	...
	public function listAllDocuments() {
		...
		if ( ! isset( $_GET['edit'] ) ) {
			...
			$document_data  = $wpdb->get_results(
				"SELECT * FROM wp_posts WHERE post_type = 'attachment' AND post_status = 'inherit' AND ID = '" . $document_id . "'",
				ARRAY_A
			);
			$tags           = $wpdb->get_results(
				'SELECT * FROM wp_document_tags ORDER BY ID DESC',
				ARRAY_A
			);
			$selected_tags  = $wpdb->get_results(
				"SELECT tag_id FROM wp_document_tags_log WHERE document_id = '" . $document_id . "'",
				ARRAY_A
			);
	...
	public function addNewtag() {
		...
		if ( ! $_GET['tag_id'] ) {
			...
			$tag_details = $wpdb->get_row(
				'SELECT * FROM wp_document_tags WHERE ID = ' . $_GET['tag_id'],
				ARRAY_A
			);
	...
	public function updateDocument( $document_id ) {
		...
		if ( isset( $_POST['update-document'] ) ) {
			...
			if ( ! wp_verify_nonce( $nonce, 'wp_edit_document' ) ) {
				...
				$deleted_tags              = $wpdb->query(
					$wpdb->prepare(
						"DELETE FROM $tags_relation_table WHERE document_id = %s",
						$document_id
					)
				);
				$delete_document_user_role = $wpdb->query(
					$wpdb->prepare(
		...
		if ( isset( $_POST['update-tag'] ) ) {
			...
			if ( ! wp_verify_nonce( $nonce, 'wp_add_tag' ) ) {
				...
				if ( ! count( $errorStatus ) ) {
					...
					// Update record in DB
					$updated_tag             = $wpdb->query(
						$wpdb->prepare(
							"UPDATE $table_name SET tag_name = %s, tag_description = %s, tag_modified = %s WHERE ID = %s",
							$tag_name,
							$description,
							date( 'Y-m-d H:i:s' ),
							$tag_id
	...
	public function insertNewTag() {
		...
		if ( isset( $_POST['create-tag'] ) ) {
			...
			if ( ! wp_verify_nonce( $nonce, 'wp_add_tag' ) ) {
				...
				$tag_name = ( isset( $_POST['tag-name'] ) ) ? sanitize_text_field( wp_unslash( $_POST['tag-name'] ) ) : ''; // WPCS: CSRF ok.
				$query    = $wpdb->prepare(
					"SELECT ID FROM $table_name WHERE tag_name = %s",
					$tag_name
				);
				if ( strlen( $tag_name ) ) {
					$wpdb->query( $query );
