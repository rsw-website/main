...
class Fusion_Images {
	...
	public function edit_grid_image_src( $html, $post_id = null, $post_thumbnail_id = null, $size = null, $attr = null ) {
		if ( ! self::$lazy_load && isset( self::$grid_image_meta['layout'] ) && in_array( self::$grid_image_meta['layout'], self::$supported_grid_layouts ) && 'full' === $size ) {
	...
	public function lazy_load_attributes( $atts, $attachment ) {
		if ( self::$lazy_load && ! is_admin() ) {

			$replaced_atts = $atts;

			if ( ! isset( $atts['class'] ) ) {
				$replaced_atts['class'] = 'lazyload';
			} else if ( strpos( $atts['class'], 'lazyload' ) ) {
				return $atts;
			} else {
				$replaced_atts['class'] .= ' lazyload';
			}
	...
	public function apply_lazy_loading( $html, $post_id = null, $post_thumbnail_id = null, $size = null, $attr = null ) {
		if ( self::$lazy_load && ! strpos( $html, 'lazyload' ) ) {
